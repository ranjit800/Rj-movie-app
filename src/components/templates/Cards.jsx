    ...
    <div className="w-full h-full px-[5%] pt-7 flex flex-wrap justify-center bg-[#1F1E24]">
      {data.map((c, i) => (
        <Link to={`/${c.media_type || title}/details/${c.id}`} className="relative w-[25vh] mr-[5%] mb-[5%] " key={i}>
          <img className=" h-[42vh] object-cover shadow-[8px_17px_38px_2px_rgba(0,0,0,.5)] rounded-lg" src={`https://image.tmdb.org/t/p/original/${c.poster_path || c.backdrop_path || c.profile_path})`} alt="" />
